 ...
 */
export class AcornParser implements ParserInterface<
  AcornNode,
  AcornJSXOpeningElement
> {
  /**
  ...
  findJSXOpeningElements(ast: AcornNode): AcornJSXOpeningElement[] {
    const elements: AcornJSXOpeningElement[] = [];
  ...
  hasDataDsAttribute(element: AcornJSXOpeningElement): boolean {
  ...
  getTagName(node: AcornJSXOpeningElement): string {
    return this.getNodeNameAsString(node);
  }

  getInsertPosition(node: AcornJSXOpeningElement): number {
