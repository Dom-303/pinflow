...
- **Classes**: PascalCase, descriptive (`PinFlowInjector`, `AnnotationService`, `ReactAdapter`) — existing compatibility names stay valid where already part of the codebase
...
- **Console messages**: `[package-name]` prefix (`[pinflow-transform]`, `[VueAdapter]`)
...
3. Internal packages: `import { PinFlowError } from '@pinflow/core'` (current compatibility layer)
...
- All domain errors use `PinFlowError` extending `Error` with RFC 7807 (Problem Details) inside the current compatibility layer
- Catch blocks use `error: unknown` — narrow with `instanceof PinFlowError` then `instanceof Error`
